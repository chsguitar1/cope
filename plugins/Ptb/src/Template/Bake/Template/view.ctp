...
<nav class="large-3 medium-4 columns" id="actions-sidebar">
    <ul class="side-nav">
        ...
        <li><?= $this->Form->postLink(__('Deletar <%= $singularHumanName %>'), ['action' => 'delete', <%= $pk %>], ['confirm' => __('Are you sure you want to delete # {0}?', <%= $pk %>)]) ?> </li>
...
foreach ($relations as $alias => $details):
    ...
    %>
    <div class="related">
        <h4><?= __('<%= $otherPluralHumanName %> Relacionados(as)') ?></h4>
...
<% endforeach; %>
            ...
            <%- $otherPk = "\${$otherSingularVar}->{$details['primaryKey'][0]}"; %>
                <td class="actions">
                    ...
                    <?= $this->Form->postLink(__('Deletar'), ['controller' => '<%= $details['controller'] %>', 'action' => 'delete', <%= $otherPk %>], ['confirm' => __('Tem certeza que deseja deletar # {0}?', <%= $otherPk %>)]) %>
                </td>
            </tr>
