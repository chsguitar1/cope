...
%>
                <td class="actions">
                    <?= $this->Html->link(__('Ver'), ['action' => 'view', <%= $pk %>]) ?>
                    <?= $this->Html->link(__('Editar'), ['action' => 'edit', <%= $pk %>]) ?>
                    <?= $this->Form->postLink(__('Deletar'), ['action' => 'delete', <%= $pk %>], ['confirm' => __('Tem certeza que deseja remover # {0}?', <%= $pk %>)]) ?>
                </td>
            </tr>
            <?php endforeach; ?>
        </tbody>
